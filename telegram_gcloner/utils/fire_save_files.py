...
class MySaveFileThread(threading.Thread):
    ...
    def run(self):
        ...
        inline_keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton(text=f'정지', callback_data=f'stop_task,{thread_id}')]])
            ...
            while True:
                ...
                if output:
                    ...
                    message_progress = '<a href="https://drive.google.com/open?id={}">{}</a>\n' \
                        ...
                        progress_total_size,
                        f'작업 속도：<code>{progress_speed_file}</code>\n' if is_fclone is True else '',
                        progress_speed,
                        progress_eta,
                    ...
                    if match:
                        message_progress = '{}\n<code>쓰기 권한 오류, 권한을 확인하십시오</code>'.format(message_progress)
                    ...
                    if match:
                        message_progress = '{}\n<code>읽기 권한 오류, 권한을 확인하십시오</code>'.format(message_progress)
                        temp_message = '{}{}'.format(message, message_progress)
                    ...
                    if self.critical_fault:
                        message_progress = '{}\n<code>사용자가 종료됨</code>'.format(message_progress)
                        process.terminate()
                        break
        ...
        update.callback_query.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton(text='완전한', callback_data='cancel')]]))
