...
def process_drive_links(update, context):
    ...
    if not folder_ids:
        return
    message = '다음 폴더가 감지되었습니다.：\n'

    try:
        gd = GoogleDrive(update.effective_user.id)
    except Exception as e:
        update.message.reply_text('SA가 올바르게 업로드되었고 즐겨찾기 폴더를 구성했는지 확인하십시오。\n{}'.format(e))
        return
    ...
    for item in folder_ids:
        try:
            ...
            update.message.reply_text('SA가 올바르게 업로드되었는지 확인하십시오，SA가 링크에 액세스할 수 있는지 확인합니다。\n{}'.format(e))
            return
    ...
    if fav_folder_ids:
        ...
        inline_keyboard_drive_ids = [[InlineKeyboardButton(text='수집되지 않은 공유드라이브，작업 전 즐겨찾기。', callback_data='#')]]
...
def save_to_folder(update, context):
    ...
    query.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(
        [[InlineKeyboardButton(text='실행되었습니다', callback_data='#')]]))
