...
def chosen_folder(update, context):
    ...
    if query.message.chat_id < 0 and \
        ...
        query.answer(text='오~쉣', show_alert=True)
        return
    ...
    try:
        ...
        context.bot.send_message(chat_id=update.effective_user.id,
                                 text='SA가 올바르게 업로드되었고 즐겨찾기 폴더를 구성했는지 확인하십시오.。\n'
    ...
    if new_fav_folders_len < max_folders:
        ...
        if not current_path_list:
            ...
            query.answer(text='오~쒯', show_alert=True)
            return
        ...
        query.answer(text='많으면 {}'.format(max_folders), show_alert=True)
    return
...
def choose_folder(update, context):
    ...
    message_id = -1
    if not query:
        rsp = update.message.reply_text('디렉토리 가져오기...')
    ...
    if query:
        ...
        if match:
            ...
            if not folders:
                ...
                context.user_data[udkey_folders_cache] = copy.deepcopy(folders)
            if not folders:
                folders = {'#': '공유드라이브가 저장되어 있지 않으면 저장해야 작동할 수 있습니다.。'}
        else:
...
def set_folders(update, context):
    ...
    page = 1
    if not query:
        rsp = update.message.reply_text('공유드라이브 가져오기...')
        rsp.done.wait(timeout=60)
        message_id = rsp.result().message_id
    else:
        if query.message.chat_id < 0 and \
    ...
    if folder_ids_len < max_folders:
        inline_keyboard_drive_ids.insert(0, [InlineKeyboardButton('즐겨찾기 폴더 추가', callback_data=callback_query_prefix)])
    inline_keyboard_drive_ids.append([InlineKeyboardButton('끝내기', callback_data='cancel')])

    context.bot.edit_message_text(chat_id=update.effective_chat.id,
                                  message_id=message_id,
                                  text='총 {}/{}개의 즐겨찾기 폴더：'.format(
                                      folder_ids_len,
                                      max_folders,
                                  ),
                                  reply_markup=InlineKeyboardMarkup(inline_keyboard_drive_ids))
